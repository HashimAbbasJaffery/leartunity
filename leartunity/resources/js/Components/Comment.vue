  ...
  <div class="comment p-4" :class="{ 'reply': comment.replies_to }" style="border-radius: 10px;">
    <div class="comment-content w-full flex">
        <div class="comment-author w-1/5 flex flex-col text-center">
            <p class="text-2xs mb-2 capitalize bg-blue-400 w-1/3 mx-auto rounded">Instructor</p>
            <img class="mx-auto border-4 border-blue-500 rounded-full bg-blue-400" :src="`/profile/${comment.user.profile.profile_pic}`" height="90" width="90"/>
            <p class="text-center text-xs mt-2">Posted {{ moment(comment.created_at, "YYYYMMDD").fromNow() }}</p>
        </div>
