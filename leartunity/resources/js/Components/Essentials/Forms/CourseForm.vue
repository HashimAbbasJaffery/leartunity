<template>
    <form @submit.prevent="submit" enctype="multipart/form-data" id="update-course" style="width: 100%; display: block;" class="py-2" method="POST" action="#">
            <label for="title" style="display: block; margin-bottom: 20px">
                Course Title
                <p class="text-red-600" v-if="!form.title" style="font-size: 13px;">{{ $page.props.errors?.title ?? "" }}</p>
            ...
            <label for="description" style="display: block; margin-bottom: 20px">
                ...
                <p class="text-red-600" v-if="!form.description || descriptionLength < 25" style="font-size: 13px;">{{ $page.props.errors?.description ?? "" }}</p>
                <p class="text-gray-500 text-sm">
                    <span v-if="descriptionLength < 25">{{ 26 - descriptionLength + ' Words required' }}</span>
                    <span v-if="descriptionLength >= 25" class="text-green-600">Done!</span>
                </p>
            ...
            <label for="pre_req" style="display: block; margin-bottom: 20px">
                ...
                <p class="text-red-600" v-if="!form.pre_req || pre_reqLength < 25" style="font-size: 13px;">{{ $page.props.errors?.pre_req ?? "" }}</p>
                <p lang="text-gray-500 text-sm">
                    <span v-if="pre_reqLength < 25">{{ 26 - pre_reqLength + ' Words required' }}</span>
                    <span v-if="pre_reqLength >= 25" class="text-green-600">Done!</span>
                </p>
            ...
            <label for="price" style="display: block; margin-bottom: 20px">
                Price
                <p class="text-red-600" v-if="!form.price" style="font-size: 13px;">{{ $page.props.errors?.price ?? "" }}</p>
...
// form.description.trim().replaceAll(/  +/g, ' ').split(' ')

let descriptionLength = computed(() => form.description.trim().replaceAll(/  +/g, ' ').split(' ').length);
let pre_reqLength = computed(() => form.pre_req.trim().replaceAll(/  +/g, ' ').split(' ').length)
